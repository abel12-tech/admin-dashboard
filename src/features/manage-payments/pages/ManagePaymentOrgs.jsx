...
const ManagePaymentOrgs = () => {
  const { isDarkMode, initializeDarkMode } = useDarkMode();
  const [currentPage, setCurrentPage] = useState(1);
  const { data: orgs, isLoading, isSuccess } = useGetAllPaymentOrgsQuery();
  const [deleteOrg] = useDeletePaymentOrgMutation();
  ...
  const totalPages = Math.ceil(orgs?.length / itemsPerPage) || 1;

  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, orgs?.length);
        ...
        <div className="w-full container h-screen p-6 overflow-y-auto rounded-lg shadow-xs">
            ...
            <table className="w-full whitespace-no-wrap">
              ...
              >
                {isLoading ? (
                  ...
                  orgs.slice(startIndex, endIndex).map((org) => (
                    <tr
                      key={org._id}
                      className={`${
          ...
          >
            <span className="flex items-center col-span-3">
              Showing {startIndex + 1}-{endIndex} of {orgs?.length}
            </span>
...
export default ManagePaymentOrgs;
